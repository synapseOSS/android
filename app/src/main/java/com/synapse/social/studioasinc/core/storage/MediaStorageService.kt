...
class MediaStorageService(
    ...
    suspend fun uploadFile(filePath: String, bucketName: String? = null, callback: UploadCallback) = withContext(Dispatchers.IO) {
        ...
        try {
            when (provider) {
                "Default" -> uploadWithDefaultProvider(file, mediaType, config, callback)
                "ImgBB" -> {
                    ...
                    uploadToImgBB(apiKey, file, callback)
                }
                "Cloudinary" -> {
                    // Use custom credentials if all are provided, otherwise use defaults
                    val cloudName = config.cloudinaryConfig.cloudName.ifBlank { DEFAULT_CLOUDINARY_CLOUD_NAME }
                    val apiKey = config.cloudinaryConfig.apiKey.ifBlank { DEFAULT_CLOUDINARY_API_KEY }
                    val apiSecret = config.cloudinaryConfig.apiSecret.ifBlank { DEFAULT_CLOUDINARY_API_SECRET }
                    
                    uploadToCloudinary(cloudName, apiKey, apiSecret, file, callback)
                }
    ...
    private suspend fun uploadWithDefaultProvider(file: File, mediaType: MediaType, config: StorageConfig, callback: UploadCallback) {
        when (mediaType) {
            MediaType.PHOTO -> {
                // Prefer custom ImgBB key if provided
                val apiKey = if (config.imgBBConfig.apiKey.isNotBlank()) {
                    android.util.Log.d("MediaStorageService", "Using custom ImgBB API key")
                    config.imgBBConfig.apiKey
                } else {
                    DEFAULT_IMGBB_API_KEY
                }
            ...
            MediaType.VIDEO, MediaType.OTHER -> {
                // Prefer custom Cloudinary credentials if all are provided
                ...
                if (hasCustomCloudinary) {
                    android.util.Log.d("MediaStorageService", "Using custom Cloudinary credentials")
                    uploadToCloudinary(
    ...
    private fun generateCloudinarySignature(timestamp: String, apiSecret: String): String {
        ...
        val hash = md.digest(toSign.toByteArray(Charsets.UTF_8))
